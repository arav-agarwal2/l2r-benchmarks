...
class CEMPlanner(ABC):

    def __init__(self, action_space=BasePlanner.default_action_space, n_planner: int = 500, horizon: int = 12, iter_update_steps: int = 3, k_best: int = 5, epsilon: float = 1e-3):
        super().__init__(action_space, n_planner, horizon)
        self.iter_update_steps = iter_update_steps
        self.k_best = k_best
        self.epsilon = epsilon
        self.lb = action_space.low
        self.ub = action_space.high
